      ...
      module BloggerAtomStreamListenerMethods
        ...
        def get_post_data_from_in_entry_elem_info
          if @in_entry_elem.nil? || !@in_entry_elem.key?(:meta) || !@in_entry_elem[:meta].key?(:kind)
            ...
            unless @in_entry_elem[:meta][:original_url].nil?
                header["redirect_from"] = URI.parse(@in_entry_elem[:meta][:original_url]).path
                puts URI.parse(@in_entry_elem[:meta][:original_url]).path
            end
...
migrator = JekyllImport::Importers::Blogger.run({"source"=> "dump.xml","no-blogger-info"=> false,"replace-internal-link" => false})